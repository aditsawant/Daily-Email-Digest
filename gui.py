...
class DailyDigestGUI:
    ...
    def __init__(self, root):
        ...
        self.__quote_var.set(self.__email.contents['quote']['include'])
        self.__wikipedia_var.set(self.__email.contents['wiki']['include']) 

        self.__sender_email_var.set(self.__email.creds['email'])
        self.__sender_password_var.set(self.__email.creds['password'])
    ...
    """
    def __build_gui_contents(self, master, quote_var, wikipedia_var):
        # create GUI widgets
        header = ttk.Label(master, text = 'Digest Contents:', style = 'Header.TLabel')
        quote_checkbox = Checkbutton(master, text = 'Quote',
    ...
    def __update_settings(self):
        ...
        self.__email.contents['quote']['include'] = self.__quote_var.get()
        self.__email.contents['wiki']['include'] = self.__wikipedia_var.get()

        self.__email.creds = {'email': self.__sender_email_var.get(),
                                           'password': self.__sender_password_var.get()}

        self.__scheduler.scheduleDaily(int(self.__hour_var.get()),
